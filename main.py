...
st.markdown("Real-time telemetry analysis using `fastf1` • Built by **you**")
...
# ──────────────────────────────────────
# Load Session Button (fresh object every time – avoids session_state bugs)
# ──────────────────────────────────────
if st.sidebar.button("Load Session", type="primary"):
    with st.spinner(f"Downloading {year} {gp} {session_type}…"):
        try:
            session = fastf1.get_session(year, gp, session_type)
            session.load(telemetry=True, laps=True, weather=True)
            st.session_state.session = session  # we store it ONLY for the current run
            st.sidebar.success(f"Loaded: {session.name}")
        except Exception as e:
            st.sidebar.error(f"Error: {e}")
            st.stop()

# Stop execution if no session loaded yet
if "session" not in st.session_state:
    st.info("Pick a race → click **Load Session** to start exploring!")
    st.balloons()
    st.stop()

# ──────────────────────────────────────
# From here on we have a REAL fastf1 Session object
# ──────────────────────────────────────
session = st.session_state.session
f1plot.setup_mpl(session=session, mpl_backend="Agg")   # required for team colors

col1, col2 = st.columns([1, 3])

# ──────────────────────────────────────
# Left column – Driver & Lap selection
# ──────────────────────────────────────
with col1:
    ...
    driver2 = st.selectbox("Driver 2", drivers, index=drivers.index("LEC") if "LEC" in drivers else 1)
...
with col2:
    ...
    # 1. Speed Trace – with proper team colors
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color=f1plot.get_team_color(session, lap1['Team']))
    ))
    fig.add_trace(go.Scatter(
        x=tel2['Distance'], y=tel2['Speed'],
        mode='lines',
        name=f"{driver2} – {lap2['Team']}",
        line=dict(color=f1plot.get_team_color(session, lap2['Team']))
    ))
    fig.update_layout(height=500, xaxis_title="Distance (m)", yaxis_title="Speed (km/h)", template="plotly_dark")
    ...
    if not race_laps.empty:
        ...
        st.info("No tyre life data available for this session.")

st.caption("Built with fastf1 • Deployed on Streamlit Cloud")

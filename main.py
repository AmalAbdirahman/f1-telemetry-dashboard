...
if st.sidebar.button("Load Session", type="primary"):
    with st.spinner(f"Loading {year} {gp} {session_type}... (first time downloads ~400 MB)"):
...
if 'session' in st.session_state:
    ...
    with col2:
        ...
        fig.update_layout(height=500, xaxis_title="Distance (m)", yaxis_title="Speed (km/h)",
                          template="plotly_dark", legend_title="Driver")
        ...
        fig_tyre.update_layout(height=600)
        st.plotly_chart(fig_tyre, use_container_width=True)

else:
    st.info("👈 Pick a race in the sidebar → click **Load Session** → start exploring!")
    st.balloons()
